...
contract BaseERC721 {
    ...
    // Mapping from token ID to approved address 单个NFT授权
    mapping(uint256 => address) private _tokenApprovals;

    // Mapping from owner to operator approvals 全局操作授权
    ...
    function mint(address to, uint256 tokenId) public {
        ...
        require(/**code*/owner == address(0), "ERC721: token already minted");
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        //调用者 应该等于token持有者 或者 等于 被授权人全局操作授权的 操作者
        require(
            /**code*/msg.sender == owner || _operatorApprovals[owner][msg.sender],
    ...
    function _isApprovedOrOwner(
        ...
        address owner = _owners[tokenId];
        require(
            /**code*/owner != address(0),
            "ERC721: operator query for nonexistent token"
        );

        /**code*/
        if(spender != owner && spender != _tokenApprovals[tokenId]){
            return false;
        }
